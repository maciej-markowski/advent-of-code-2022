...
in the datastream. In the protocol being used by the Elves, the start of a packet is indicated by a sequence
...
def findIndex(iLength) -> str:
  # today's input is single line
  with open('input', 'r') as fInputFile:
    sStream = fInputFile.readline()

  iStreamIndex = iLength
  iStreamLength = len(sStream)
  asMarker = [*sStream][0:iLength]

  while iStreamIndex < iStreamLength:
    if len(asMarker) == len(set(asMarker)):
      return str(iStreamIndex)
    asMarker.pop(0)
    asMarker.append(sStream[iStreamIndex])
    iStreamIndex += 1


def taskOne() -> None:
  print("Marker found after receiving {} characters.".format(findIndex(4)))

"""
--- Part Two ---
Your device's communication system is correctly detecting packets, but still isn't working. It looks like it also needs
to look for messages. A start-of-message marker is just like a start-of-packet marker, except it consists of 14 distinct
...
def taskTwo() -> None:
  print("Mesage marker found after receiving {} characters.".format(findIndex(14)))
