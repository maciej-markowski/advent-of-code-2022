...
finding the errors. Every item type is identified by a single lowercase or uppercase letter (that is,
...
def taskOne():
  ...
  with open('input', 'r') as fInputFile:
    while True:
      ...
      if not sLine:
        print("Total sum of priorities: {}".format(str(iSumOfPriorities)))
        break
      sFirstCompartment = sLine[:len(sLine)//2]
      sSecondCompartment = sLine[len(sLine)//2:]

      for sItem in sFirstCompartment:
        if sItem in sSecondCompartment:
          iSumOfPriorities += itemToPriority(sItem)
          break
...
def taskTwo():
  iSumOfBadgePriorities = 0
  with open('input', 'r') as fInputFile:
    while True:
      sFirstRucksack = fInputFile.readline()
      # for this task only we know that input file will have 300 lines exactly
      if not sFirstRucksack:
        print("Total sum of badge priorities: {}".format(str(iSumOfBadgePriorities)))
        break
      sSecondRucksack = fInputFile.readline()
      sThirdRuckSack = fInputFile.readline()

      for item in sFirstRucksack:
        if item in sSecondRucksack and item in sThirdRuckSack:
          iSumOfBadgePriorities += itemToPriority(item)
          break
