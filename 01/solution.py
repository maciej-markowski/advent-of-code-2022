...
with open('input', 'r') as fInputFile:
  while True:
    ...
    if not sLine:
      print(f"Max Calories: {iMaxCalories} carried by elf number: {iMaxElfID}")
      break
    if sLine == '\n':
      if iCurrentCalories > iMaxCalories:
        iMaxElfID = iCurrentElfID
        iMaxCalories = iCurrentCalories
      iCurrentElfID += 1
      iCurrentCalories = 0
      continue
    iCurrentCalories += int(sLine)

print('############################################################################################################')
"""
--- Part Two ---
By the time you calculate the answer to the Elves' question, they've already realized that the Elf
carrying the most Calories of food might eventually run out of snacks. To avoid this unacceptable situation,
...
iCurrentElfID = 1
aoCaloriesInventory = [ ]
iTop3TotalCalories = 0

with open('input', 'r') as fInputFile:
  while True:
    sLine = fInputFile.readline()
    if not sLine:
      aoCaloriesInventoryTop3 = sorted(aoCaloriesInventory, key=lambda k: k['CaloriesCarried'], reverse=True)[0:3]
      for dElf in aoCaloriesInventoryTop3:
        iTop3TotalCalories += dElf['CaloriesCarried']
      print("Total calories carried by top 3 elves is {}.".format(str(iTop3TotalCalories)))
      break
    if sLine == '\n':
      aoCaloriesInventory.append({'ElfId': iCurrentElfID, 'CaloriesCarried': iCurrentCalories})
      iCurrentElfID += 1
      iCurrentCalories = 0
      continue
    iCurrentCalories += int(sLine)
