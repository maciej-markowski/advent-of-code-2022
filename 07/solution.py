...
Since the disk is full, your first step should probably be to find directories that are good candidates for deletion. To do this, you need to determine
the total size of each directory. The total size of a directory is the sum of the sizes of the files it contains, directly or indirectly.
...
def taskOneAndTwo() -> None:
    ...
    while True:
      ...
      if sLine.startswith('$ cd'):
        if sLine.split()[2] == '..':
          ...
          oAllDirs[sPath] = 0
      # ls output
      elif sLine.startswith('dir') or sLine.startswith('$ ls'):
        continue
      # file
      else:
        p = sPath
        while True:
          if p == '' or p =='/':
            oAllDirs['/'] += int(sLine.split()[0])
            break
          oAllDirs[p] += int(sLine.split()[0])
          p = re.sub(r'\/([a-z])*$', '', p)

  iTotalSmalDirSize = 0

  for size in oAllDirs.values():
    if size <= 100000:
      iTotalSmalDirSize += size

  print("Sum of total sizes for small dirs is: {}".format(str(iTotalSmalDirSize)))
  ...
  iToDelete = 30000000 - (70000000 - oAllDirs['/'])

  aiCandidatesToDelete = []
  for size in oAllDirs.values():
    if size >= iToDelete:
      aiCandidatesToDelete.append(size)

  print("Smallest directory to delete that would solve the problem has size of {}".format(str(min(aiCandidatesToDelete))))
