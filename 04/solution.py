...
def taskOne() -> None:
  ...
  with open('input', 'r') as fInputFile:
    while True:
      ...
      if not sLine:
        print("Total paris with one range fully contain the other: {}".format(str(iOverlapingPairs)))
        break
      aFirstElf['min'] = int(sLine.split(',')[0].split('-')[0])
      aFirstElf['max'] = int(sLine.split(',')[0].split('-')[1])
      aSecondElf['min'] = int(sLine.split(',')[1].split('-')[0])
      aSecondElf['max'] = int(sLine.split(',')[1].split('-')[1])
      if (
          (aFirstElf['min'] >= aSecondElf['min'] and aFirstElf['max'] <= aSecondElf['max']) or
          (aFirstElf['min'] <= aSecondElf['min'] and aFirstElf['max'] >= aSecondElf['max'])
        ):
        iOverlapingPairs += 1
...
def taskTwo() -> None:
  ...
  with open('input', 'r') as fInputFile:
    while True:
      sLine = fInputFile.readline()
      if not sLine:
        print("Total overlaping paris: {}".format(str(iOverlapingPairs)))
        break
      aFirstElf['min'] = int(sLine.split(',')[0].split('-')[0])
      aFirstElf['max'] = int(sLine.split(',')[0].split('-')[1])
      aSecondElf['min'] = int(sLine.split(',')[1].split('-')[0])
      aSecondElf['max'] = int(sLine.split(',')[1].split('-')[1])
      if (
        (aSecondElf['min'] <= aFirstElf['min'] <= aSecondElf['max']) or
        (aFirstElf['min'] <= aSecondElf['min'] <= aFirstElf['max'])
      ):
        iOverlapingPairs += 1
